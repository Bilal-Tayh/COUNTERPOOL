...
uint64 MurMurHash(T key, uint64_t seed) 
{
    ...
    k = (char*) memcpy(k,&key,sizeof(T));
    uint32_t r;
    MurmurHash3_x86_32_1(k,sizeof(T),seed,&r);
    free(k);
    return r;
}
...
int CPCF<key_type>::myhash(key_type key, int i, int s) {
    uint64_t   val=0;
...
template <typename key_type>
CPCF<key_type>::CPCF(int M,int f, int slot, int hash_type_param)
{
  initLookupTableFunc();
  cf_size=M;
  fp_size=(1<<f);
...
template <typename key_type>
void CPCF<key_type>::initLookupTableFunc()
{
...
template <typename key_type>
uint32_t CPCF<key_type>::decode_sizes(uint16_t encoding, uint64_t n, uint64_t k)
{
	/*
...
template <typename key_type>
uint32_t CPCF<key_type>::decode_offsets(uint16_t encoding, uint64_t n, uint64_t k)
{
...
template <typename key_type>
uint16_t CPCF<key_type>::encode(uint32_t sizes)
{
	uint64_t n = 64;
	uint64_t k = 4;
	uint16_t encoding = 0;
...
template <typename key_type>
uint16_t CPCF<key_type>::encode_without_auxilary_map(uint32_t sizes)
{
	uint64_t n = 64;
	uint64_t k = 4;
...
template <typename key_type>
bool CPCF<key_type>::incPool(int index, int counterNum , uint64_t value){
  ...
  int newSizeBits = ceil(log2(value+oldVal));

  int oldSizeBits = offsets[counterNum + 1] - offsets[counterNum];

  int diff  = newSizeBits - oldSizeBits;

  //check if the last counter will have enough space
  int lastCounterBitOffset = offsets[k-1] - diff;
  int lastCounterVal =  pool >> lastCounterBitOffset;
  if(  64 - lastCounterBitOffset < ceil(log2(lastCounterVal))){
    return false;
  }
  ...
  for(int i=0; i< value;i++){
    incPoolCounter(index, counterNum);                                                  
  }
...
template <typename key_type>
void CPCF<key_type>::incPoolCounter(int index, int counterNum){
...
template <typename key_type>
uint64_t CPCF<key_type>::getVal(int index, int counterNum){
...
template <typename key_type>
uint64_t CPCF<key_type>::delCounter(int index, int counterNum){
 ...
 */
template <typename key_type>
bool CPCF<key_type>::insert(key_type key)
{
...
template <typename key_type>
bool CPCF<key_type>::insert2(key_type key,int p,int fingerprint)
{
  int t;
  int new_bit=0;
  int old_bit=0;
  ...
  for (t = 1;  t <= 1000;  t++) {
    ...
    // check if the fingerprint exist in one of the two pins
    ...
    __m256i binsContent = _mm256_set_m128i( bin1Content, bin2Content);
    ...
    const __m256i item2 = _mm256_set1_epi32((int)-1);
    const __m256i match2 = _mm256_cmpeq_epi32(item2, binsContent);
...
template <typename key_type>
bool CPCF<key_type>::direct_insert(key_type key)
{
  if (cf_size==0) return true;
    if (query(key)) {
...
template <typename key_type>
bool CPCF<key_type>::direct_insert(const int p_in, const int f_in)
{
  if (cf_size==0) return true;
...
template <typename key_type>
bool CPCF<key_type>::query(const key_type key)
{
...
template <typename key_type>
std::pair<int,int> CPCF<key_type>::fullquery(const key_type key)
{
...
template <typename key_type>
std::pair<int,int> CPCF<key_type>::get_pf(const key_type key) {
    int fingerprint=myhash(key,1,fp_size);
    fingerprint= fingerprint % fp_size;
    int p=myhash(key,2,cf_size);
...
//template class CPCF<five_tuple>;
template class CPCF<int>;