...
// add in the CPCF.cpp the template class eg:


template <typename key_type> class CPCF {
		//int ***table;//  memory
		int **keys;//  memory
		int **table;//  memory
		...
		public:
		CPCF(int M,int f, int slot, int hash_type_param);
		//CF(int M,int f);
		virtual ~CPCF();
		void clear();
		void dump();
		...
		private:
		void incPoolCounter(int index, int counterNum);
